...
public:
	...
	void draw() {
		...
		for (float i = 0; i < idokul; i += 0.001) {
	...
	void korbemegy(float d) {
		for (float i = 0; i < d; i += 0.001) {
			mozgas(0.001);
			forgas(0.0025);
		}
	}
...
Hami zold = Hami(vec3(0, 1, 0), vec2(0.9, 0.8));
Hami piros = Hami(vec3(1, 0, 0), vec2(-0.9, 0.8));
...
void onIdle() {
	...
	idokul = sec - utolsoido;
	zold.korbemegy(sec - utolsoido);
	utolsoido = sec;
